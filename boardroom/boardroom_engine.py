...
class BoardroomEngine(ABC):
    ...
    def executeMandatedToolStage(self, agent: FinancialAgent, initialPrompt: str, mandatedToolName: str, config: BoardroomConfig,
        subrole: Optional[str] = None, maxRetries: Union[Tuple[int, int], int] = (8, 4), summarisationOverride: Optional[bool] = None, requireInitialTools: bool = True,
        confirmationPrompt: Optional[str] = None, modeOverride: Optional[str] = None) -> Tuple[str, str]:

        if isinstance(maxRetries, (tuple, list)):
            toolRetries, maxResets = int(maxRetries[0]), int(maxRetries[1])
        else:
            toolRetries, maxResets = int(maxRetries), 4
        ...
        historyResetCount = 0
        originalMessageHistory = [msg.copy() for msg in agent.messageHistory]
        ...
        while True:
            ...
            for attempt in range(toolRetries + 1):
                if isStopRequested():
                    raise SimulationStoppedException("Simulation stopped by user.")

                isLastAttempt = (attempt == toolRetries)
                ...
                if not isLastAttempt:
                    ...
                    if toolRetries - attempt <= 3:
                        currentPrompt += (
                            f"\n\nPlease note that you have {toolRetries - attempt} attempts remaining to submit a valid '{mandatedToolName}' tool call. "
            ...
            if historyResetCount > maxResets:
                ...
                errorMsg = (
                    ...
                    f"after {toolRetries} retries and {maxResets} history resets. Please try a larger parameter model or adjust generation settings."
                )
