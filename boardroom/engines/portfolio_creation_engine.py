...
class CreationSectorDebateStage(BoardroomStage):
    ...
    def execute(self, context: BoardroomContext) -> Optional[str]:
        ...
        bearPrompt = (
            ...
            f"- {promptArgs['sectorDiversityRule']}\n"
            f"- Max single sector allocation: {promptArgs['maxSectorAllocation']}\n\n"
            f"1. You MUST first call 'fetchAllSectorsAnalysis' to evaluate all 11 GICS sectors. Scrutinise narrow rallies (where ETF return >> median constituent return), deteriorating breadth (< 50% above 50 SMA), high channel valuation extremes, 10Y Treasury yield vulnerability, and bearish sentiment.\n"
            f"2. Present a clear table of percentage allocations across your selected sectors summing to 100.0%.\n"
    ...
    def execute(self, context: BoardroomContext) -> Optional[str]:
        ...
        if pace == BoardroomPace.PRESET:
            ...
            aggProposalPrompt = (
                ...
                f"Task: Review and scrutinise BOTH the Growth Hunter candidates and Defensive/Value Hunter candidates from an aggressive, high-upside risk perspective. Construct your final aggressive individual stock allocation proposal for {promptArgs['initialCapital']}.\n\n"
                f"Instructions:\n"
                f"1. Scrutinise candidates from both hunters, identifying high-beta opportunities with strong upside momentum and earnings catalysts, while challenging overly sluggish picks.\n"
                f"2. Group your stock proposals strictly under each confirmed sector above, assigning whole integer 'perSectorWeight' percentages summing strictly to 100% per sector.\n"
            ...
            consProposalPrompt = (
                ...
                f"Growth Candidates Scouted in Phase 4:\n{growthRaw}\n\n"
                f"Defensive Candidates Scouted in Phase 4:\n{valueRaw}\n\n"
                f"Task: Review and scrutinise BOTH the Growth Hunter candidates and Defensive/Value Hunter candidates from a conservative, risk-managed capital-preservation perspective. Construct your final conservative individual stock allocation proposal for {promptArgs['initialCapital']}.\n\n"
                f"Instructions:\n"
                f"1. Scrutinise candidates from both hunters, prioritizing rock-solid balance sheets, dividend reliability, low volatility, and margin of safety, while challenging speculative or overvalued picks.\n"
                f"2. Group your stock proposals strictly under each confirmed sector above, assigning whole integer 'perSectorWeight' percentages summing strictly to 100% per sector.\n"
